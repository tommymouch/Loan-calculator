...
public class LoanCalcViewController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		LoanAmount.setText("50000");
		InterestRate.setText("7");
		NbrOfYears.setText("20");
		AdditionalPayment.setText("200");
	...
	private void toggleEscrow(String strLoanType) {
		...
		this.esc = (strLoanType == "Home") ? true : false;
	}
	...
	private boolean ValidateData() {
		...
		TextField validateAlpha[] = {LoanAmount, InterestRate, NbrOfYears, EscrowAmount, AdditionalPayment};
		int switchCnt = 2;
		
		for (TextField t : validateAlpha) {
			if (t.getText().trim().matches("[^0-9]")) {
				//System.out.println("meche el hal///////////\n\n\n\n\n");
				switchCnt = 1;
				continue;
			}
		...
		switch (switchCnt) {
		case 1:
			contentText.append("Invalid Input: Alpha characters are not allowed. Please enter numeric values (integers or decimals) only. \n");
			goodtogo = false;
			break;
		...
		case 2:
			...
			if (InterestRate.getText().trim().isEmpty() || Double.parseDouble(InterestRate.getText().trim()) <= 0 || Double.parseDouble(InterestRate.getText().trim()) > 20) {
				contentText.append("Interest Rate must be a positive double between 0 and 20. \n");
				goodtogo = false;
			}
